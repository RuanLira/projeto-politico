...
export const publications: Publication[] = [
  ...
  {
    id: 'pub-2',
    title: 'Visita técnica ao Hospital Municipal',
    platform: 'Instagram',
    contentType: 'Vídeo',
    date: '2026-06-11',
    reach: 95300,
  ...
  {
    id: 'pub-4',
    title: 'Audiência pública sobre mobilidade',
    platform: 'X',
    contentType: 'Texto',
    date: '2026-06-23',
    reach: 43800,
...
const names = [
  ...
  ['@carla_saude', 'Carla Nunes', 'São Paulo'],
  ['@diegoambiental', 'Diego Prado', 'Sorocaba'],
  ['@elisa_mobilidade', 'Elisa Rocha', 'Osasco'],
  ['@fernando.sp', 'Fernando Melo', 'Guarulhos'],
  ['@giovana_comunica', 'Giovana Sales', 'São Bernardo'],
  ['@heitorlocal', 'Heitor Ramos', 'Jundiaí'],
  ['@inesparticipa', 'Ines Vieira', 'Ribeirão Preto'],
  ['@joao_bairro', 'Joao Martins', 'São Paulo'],
  ['@karina_causa', 'Karina Costa', 'Diadema'],
  ['@lucas_transparente', 'Lucas Dias', 'Barueri'],
  ['@marcia_popular', 'Marcia Freitas', 'Mauá'],
  ['@nando_agenda', 'Nando Silva', 'São Caetano'],
  ['@olivia_digital', 'Olivia Pires', 'Cotia'],
  ['@paulo_comunidade', 'Paulo Ferraz', 'Taboão'],
...
export const publicProfiles: PublicProfile[] = names.map(([handle, name, city], index) => ({
  ...
  tags: index % 3 === 0 ? ['liderança local', 'recorrente'] : index % 3 === 1 ? ['educacao'] : ['saude'],
}));

const interactionTypes = ['curtida', 'comentário', 'compartilhamento', 'menção'] as const;
  ...
  return Array.from({ length: count }).map((_, interactionIndex) => ({
    ...
    text:
      interactionIndex % 3 === 0
        ? 'Interação pública registrada em conteúdo do mandato.'
        : undefined,
  }));
});

export const teamUsers: TeamUser[] = [
  { id: 'user-1', name: 'Ruan Lira', email: 'admin@controlepolitico.com', role: 'Administrador', status: 'Ativo', lastAccess: '2026-07-02 09:12' },
  { id: 'user-2', name: 'Camila Rocha', email: 'gabinete@controlepolitico.com', role: 'Gabinete', status: 'Ativo', lastAccess: '2026-07-01 18:44' },
  { id: 'user-3', name: 'Felipe Maia', email: 'comunicacao@controlepolitico.com', role: 'Comunicação', status: 'Ativo', lastAccess: '2026-07-02 08:33' },
  { id: 'user-4', name: 'Livia Costa', email: 'consulta@controlepolitico.com', role: 'Consulta', status: 'Pendente', lastAccess: '-' },
];

export const auditLogs: AuditLog[] = [
  { id: 'log-1', user: 'Ruan Lira', action: 'Gerou relatório mensal', target: 'Relatórios', date: '2026-07-02 09:22', severity: 'success' },
  { id: 'log-2', user: 'Camila Rocha', action: 'Editou permissão de usuário', target: 'Equipe', date: '2026-07-01 17:18', severity: 'warning' },
  { id: 'log-3', user: 'Felipe Maia', action: 'Exportou ranking em CSV', target: 'Ranking', date: '2026-07-01 15:03', severity: 'info' },
  { id: 'log-4', user: 'Sistema', action: 'Bloqueou tentativa excessiva de login', target: 'Autenticação', date: '2026-06-30 21:46', severity: 'warning' },
];
