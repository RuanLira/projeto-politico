...
const demoUsers: Array<AppUser & { password: string }> = [
  { id: 'auth-1', name: 'Ruan Lira', email: 'admin@controlepolitico.com', role: 'Administrador', password: 'Admin123' },
  { id: 'auth-2', name: 'Equipe Comunicação', email: 'comunicacao@controlepolitico.com', role: 'Comunicação', password: 'Comunicacao123' },
...
export async function login(email: string, password: string): Promise<AppUser> {
  ...
  if (!isStrongEnoughPassword(password)) {
    throw new Error('Use uma senha com ao menos 8 caracteres, uma letra maiúscula e um número.');
  }
